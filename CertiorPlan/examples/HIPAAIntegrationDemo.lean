...
#widget CertiorPlan.verificationExplorerWidget with brokenWidgetProps

-- Place cursor here → infoview shows the fixed plan.
-- This one completes successfully — redact before output.
def fixedWidgetProps : CertiorPlan.WidgetInitProps := { planInfo := patientPipelineFixed }
#widget CertiorPlan.verificationExplorerWidget with fixedWidgetProps
