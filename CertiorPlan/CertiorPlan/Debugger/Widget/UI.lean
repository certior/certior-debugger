...
function CertificatesPanel({ certificates }) {
  ...
  const rows = certificates.map((cert, i) => {
    const verified = cert.verified !== false;
  ...
  const allOk = certificates.every(c => c.verified !== false);
...
function FlowGraphPanel({ flowEdges }) {
  ...
  const rows = flowEdges.map((edge, i) => {
    ...
    return e('div', { key:i, style:{
      ...
      background: ok ? 'transparent' : '#fef2f2'
    }}, [
      e('span', { key:'src', style:{ fontWeight:600, fontSize:'11px' }}, '['+edge.source+']'),
      e('span', { key:'arrow', style:{ color: ok ? '#0e7490' : '#dc2626', fontWeight:700, fontSize:'11px' }}, ' ──'+edge.sourceLevel+'→ '),
      e('span', { key:'tgt', style:{ fontWeight:600, fontSize:'11px' }}, '['+edge.target+']'),
